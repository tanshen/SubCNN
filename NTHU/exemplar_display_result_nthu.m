function exemplar_display_result_nthu
...
filename = sprintf('%s/detections_%s.txt', result_dir, image_set);
...
for i = 1:N
    ...
    file_img = sprintf('%s/%s/%s.jpg', root_dir, image_set, img_idx);
    I = imread(file_img);
    ...
    hold on;
    for k = 1:num
        if det(k,6) > threshold_people && strcmp(det_cls{k}, 'Car') == 0
            % get predicted bounding box
            bbox_pr = det(k,1:4);
    ...
    if is_save
        filename = fullfile(dirname, sprintf('%s.png', img_idx));
        % saveas(hf, filename);
