...
LOG="experiments/logs/kitti_val_caffenet_rcnn_msr_fix.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./tools/test_net.py --gpu $1 \
  ...
  --net output/kitti/kitti_train/caffenet_fast_rcnn_msr_fix_kitti_iter_40000.caffemodel \
  --imdb kitti_val \
  --cfg experiments/cfgs/kitti_rcnn_msr_fix.yml
