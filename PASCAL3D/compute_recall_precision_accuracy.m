...
path_ann_view = sprintf('%s/Annotations', opt.root);
...
% load results
filename = sprintf('results/det_val_%s.txt', cls);
[ids_det, confidence, azimuth_det, b1, b2, b3, b4] = textread(filename, '%s %f %f %f %f %f %f');
...
for i = 1:M
    ...
    index = strcmp(ids{i}, ids_det);
    dets = [b1(index) b2(index) b3(index) b4(index) azimuth_det(index) confidence(index)];
    num(i) = size(dets, 1);
