function exemplar_display_result_pascal3d
...
opt = globals;
pascal_init;
[ids, t] = textread(sprintf(VOCopts.imgsetpath, 'val'), '%s %d');
N = numel(ids);

% load data
filename = fullfile(opt.SLMroot, 'PASCAL3D/data.mat');
...
for i = 1:N
    ...
    file_img = sprintf(VOCopts.imgpath, img_idx);
    I = imread(file_img);
    ...
    for k = 1:num
        if det(k,6) > threshold
            ...
            if data.trunc_per(cid) > 0 && isempty(index) == 0
            ...
            % show segments
            ind_cls = find(strcmp(det_cls{k}, classes) == 1);
            index_color = 1 + floor((ind_cls-1) * size(cmap,1) / numel(classes));
            dispColor = 255*cmap(index_color,:);
            scale = round(max(size(I))/200);            
